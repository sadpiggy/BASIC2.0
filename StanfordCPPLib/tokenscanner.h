 ...
 * a string into individual logical units called <b><i>tokens</i></b>.//<>啥意思啊？
 */
...
enum TokenType { SEPARATOR, WORD, NUMBER, STRING, OPERATOR };//separator和operator不一样吗?
 ...
 * <code>input</code>://input is the all?
 *
 *<pre>
 ...
 *        TokenScanner scanner(infile);//??放一个文件进去？
 * ------------------------------------
 ...
 * --------------------------------
 * Sets the token stream for this scanner to the specified(明确说明的) string or
 * input stream.  Any previous token stream is discarded//丢弃.
 */
//莫非是更新为新的input
 ...
 * token stream.//塞进去，然后下一次使用nexttoken时直接将它取出//那若果我多次save...呢？
 */
 ...
 * to the beginning of the saved token.//啥几把？/////////////////////  If <code>saveToken</code> is
 ...
 * both the slash-star and slash-slash comment format from the C-based//忽略注释？
 ...
 * longest substring that can be interpreted as a real number.//so,maybe spmething that 2.0can be interpret as a nember,but illegal
 */
