 ...
 * ----------------------------------------------------
 * Adds the parsed representation of the statement to the statement//读不太懂
 ...
 */
    ...
    int getFirstLineNumber(){if(Lines.empty()==true)return -1;    return Lines.begin()->first;}//empty==ture???
 ...
 */
   ...
   bool havecontained(int line_number){
       if(first_line==-1)return false;//空表直接false
       if(Lines.find(line_number)==Lines.end())return false;
       return true;
   }
   void Showlist();
   void RunProgram(EvalState &evalstate);
private:
...
struct Line{
    ...
    Line()=default;
    Line(string& it):sourse_line(it){}
    Line(string& it1,Statement* it2):sourse_line(it1),stmt(it2){}
    //bool has_been_deleted=false;//判断是否被DELETEd
};
    map<int,Line>Lines;
    int first_line=-1;
   // bool is_ended=false;
