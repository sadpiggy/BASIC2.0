...
int IdentifierExp::eval(EvalState & state) {
   if (!state.isDefined(name)) error("VARIABLE NOT DEFINED");//有问题
 ...
 */
//那expresions的树是怎么建的？是不是还分优先级？是什么划分exp?
...
int CompoundExp::eval(EvalState & state) {
   if (op == "=") {
      ...
      int val = rhs->eval(state);//怎么不直接返回value你？晃我》。。。
      state.setValue(((IdentifierExp *) lhs)->getName(), val);//map装值
      return val;
   }
   ...
   if (op == "/") {
       ...
       return left / right;}
  // error("Illegal operator in expression");//特殊类型的error不是只有4种吗？这是个什么鬼？
    error("SYNTAX ERROR");
   return 0;
}
